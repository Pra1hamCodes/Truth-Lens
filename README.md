...
```bash
git lfs install
git clone https://github.com/Pra1hamCodes/Truth-Lens.git
cd Truth-Lens
```
...
- **Render** reads `render.yaml` and provisions three services: `truth-lens-backend` (Node), `truth-lens-flask-cnn` (Python), `truth-lens-flask-vit` (Python). All three respect `$PORT`. The `.pt` model is pulled via Git LFS during the build.
